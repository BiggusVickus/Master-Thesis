...
There are various features that a researcher will be looking for in a lab. 
A combination of these features results in a graph being called "good". 
...
Those specific plots can be classed as a "good curve" due to the plots exhibiting a clear growth, peak, and death cycle as explained in 

The resources have been consumed by $t=8$, coinciding with the time at which the phages population starts to peak and which the total bacteria population is quickly approaching 0. 
Once the resources reach 0, no more bacteria are being created. 

The uninfected and infected bacteria exhibit exponential growth, peaking at 1617 uninfected bacteria at $t=3.99$ and 3463 infected bacteria at $t=5.27$ respectively. 
At around $t=4$ is when the the resource really start to deplete as that is the peak of the uninfected bacteria. 
At $t=5$ is when the resource consumption rate starts to slow down as from then on more bacteria are being killed than being created. 
This effect creates a decreasing sigmoid shape in the resource concentration. 
The bacteria sum do not have as stark of a peak in comparison to the uninfected and infected bacteria, due to the graph measuring all bacteria populations, but the peak of 3805 at $t=4.89$ is still clear and pronounced.

The phages saw a significant increase in population count at around $t=4$, coinciding with the peak in uninfected bacteria. 
There is a delay between the peak of bacteria at $t=5$ and when the phag epopulation reahces its effective max population at $t=10$. 
