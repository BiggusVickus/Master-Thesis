...
We randomly selected parameter values from a uniform distribution within the ranges used for the SOBOL analysis (\Cref{tab:appendixE:SOBOL_analysis_values}). 
...
In the default selected parameter values, there is exactly one bacteria population that manages to survive, while the others all die out. 
This can be seen in \Cref{fig:created:no_debris_model}. 
