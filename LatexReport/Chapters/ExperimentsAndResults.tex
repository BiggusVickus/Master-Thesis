...
\Cref{fig:created:r_beta_washout_0}, \Cref{fig:created:r_beta_washout_0}, and \Cref{fig:created:r_beta_washout_0}, show a 7x7 matrix of different $r$ and $\beta$ initial conditions for a $3\times2\times3$ model. 
For each simulation, if $r$ or $\beta$ is equal to a value not equal to $inf$, as identified by the title above each subfigure, then each value of $r$ or $\beta$ has that value.
...
As a small example for the $3\times 2\times 3$ model, when $r=0.200$, the simulation framework uses $r = \left(\begin{smallmatrix}\textcolor{red}{0.200} & 0.200 \\ 0.200 & \textcolor{red}{0.200} \\ 0.200 & 0.200 \\ \end{smallmatrix}\right)$ as the input matrix to the ODE model. 
It is important that in the user-implemented ODE model, the user needs to check if an edge exists between two nodes first before calculating the new values. 
The values 

When $r=inf$, the simulation framework uses $r=\left(\begin{smallmatrix}0 & 0.11695 \\ 0.144459 & 0 \\ 0.11895 & 0.13065 \\ \end{smallmatrix}\right)$ as the data to simulate the interactions with. 