...
\Cref{fig:created:phase_portrait_resource_phage_max_phage_proliferate} simulates a wider range of values. 
...
By varying the initial resource concentration, the bacteria growth rate is affected, in turn affecting the phage population, where the effect is non linear. 
There is diminishing returns, where increasing initial phage populations doesn't drastically increase the max phage value reached, but is rather a function of the initial resource concentration. 
However, more initial resources will be required in order to ensure the phage population proliferates. 
TODO: ASK MATTI ABOUT PRICE OF PHAGES VS RESOURCES/NUTRIENTS (IN GENERAL)
...
\begin{figure}[]
    ...
    \begin{subfigure}{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Plots/Created/PP/phase_portrait_resources_phage_max_phage_proliferate.png}
        \caption{
            Phage population proliferation as a function of initial resource and phage concentrations. 
            While the color appears uniform along the vertical axis, each cell represents a slightly different value. 
        }
        \label{fig:created:phase_portrait_resource_phage_max_phage_proliferate}
    \end{subfigure}
    \caption{
        Varying initial resources and initial phages and the resulting curve and proliferation. 
        The boundary of proliferation follows a logarithmic curve. 
