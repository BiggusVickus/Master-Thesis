...
\begin{eqfloat}
    ...
    \caption{
        ...
        The text in red has been added to the model, adding (the wash-in) fresh resources ($\omega^i$) and the removal (wash-out) of agents ($\omega^o$). 
        ...
        A summary of the parameters can be found at \Cref{tab:parameter_table_simple_golden_model}. 
    }
\end{eqfloat}
...
When resource $R_1$ is being consumed by bacteria $B_1$ and $B_2$, $R_1$ is reduced by the sum of the resources used by $B_1$ and $B_2$. 
Likewise, if bacteria $B_1$ is being infected by phage $P_1$ and $P_2$, then the uninfected $B_1$ population is reduced by the sum of infections from $P_1$ and $P_2$. 
\begin{eqfloat}
    \begin{align}
...
When $ST_i >> S1_i$ then parameter $i$ depends on interactions with other parameters, while when $ST_i \approx S1_i$, then $i$ doesn't interact much with and depend on other parameters.
...
Another argument is that the simple golden model doesn't exhibit complex behavior unlike the oscilalting behavior exhibited in \Cref{fig:sourced:cocktail_and_phagedyn}. 
