...
\begin{align}
    \frac{dN_n}{dt} &= \sum_{b \in B} -e_{b, n} \cdot g(N_n, b)\cdot (U_b + \sum_{i=1}^{M} I_{i_b})\\
    \frac{dU_b}{dt} &= U_b \cdot \sum_{b \in B} g(N_n, b)\cdot - U_b \cdot (\sum_{p \in P} r_{p, b} \cdot P_p)\\
    \frac{dI_{1_b}}{dt} &= U_b \cdot (\sum_{p \in P}r_{p, b} \cdot P_p) - \frac{M}{\tau_b}\cdot I_{1_b} \\
    \frac{dI_{k_b}}{dt} &= \frac{M}{\tau}(I_{k-1_b}-I_{k_b}) \text{ for } k=2, \dots, M \\
    \frac{dP_p}{dt} &= B_{p, b}\cdot\frac{M}{\tau_b} \cdot I_{M_b} - r_{p, b}\cdot(U_b + \sum_{i=1}^{M} I_{M_b})\cdot P_p \\
