...
The final part allows for the user to download the simulation data to create their own custom graphs and analyses. 

A flowchart acting as a bird eye view of interactions between the user and systems can be seen in \nameref{AppendixC}. 
...
The user can edit the values of the attributes in \nameref{sec:part3}, so the parameter values do not have to be perfect. 
...
In the network model you would explicitly create a $2\times 3 \times 2$ network, but when passing the network to the simulation framework, you would tell the framework to model the subagents. 
...
The program was created exclusively in Python, and makes extensive usages of various packages, ranging from the standard scientific packages such as NumPy and SciPy to more niche packages such as pickle and SALib \cite{iwanagaSALib20Advancing2022, hermanSALibOpensourcePython2017}. \newline
