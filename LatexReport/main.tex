...
\abstract{\addtocontents{toc}{\vspace{1em}} 
    The interactions between phages, bacteria, resources, and the environment are complex. 
    Not much is known about how phages interact with bacteria and their surroundings. 
    There is a need to mathematically model phage and bacteria populations, but current models can only model 1 or 2 phages, bacteria strains, and resources. 
    In the marine ecosystem, there can be hundreds of different phages and bacteria interacting with one another. 
    These individual interactions have a greater implication on the ecosystem. 
    Phages have been shown to influence bacteria's population fitness levels by introducing mutations and forcing bacteria to adapt to external stressors. 
    Phages have also been shown to influence the resource recycling process. 
    Upon lysing, bacteria release resources into the environment that other bacteria, plants, and animals can use. 

    I created a program to create a tool that will create a network representation of the model's interactions. 
    The graph network stores interactions and model parameters such as initial population and reaction rates, as well as environmental conditions like temperature and pH. 
    I also created a simulation framework that can mathematically model the phage, bacteria, and resource population with the provided network, with a user-provided ODE model. 
    The user can interact with the framework with a custom-built dashboard. 
    Using the tools, a user can upload a self-implemented ODE model and network model to a dashboard and a self-implemented ODE model and interactively run simulations on the model. 
    The user can, for example, change the initial resource concentration or how fast a phage infects a bacterium from the dashboard and immediately see how the change affects the simulation. 
    The user can interact with five prebuilt plots. 
    The user can run the simulations and save the results to their disk to create their own custom analyses and plots. 

    Finally, I analyze a model using the Golding model from \citet{mullaExtremeDiversityPhage2024}. 
    I qualitatively analyze the model's behavior and use a SOBOL analyses to help assign a quantitative value to the model's behavior. 
...
% A table of Symbols and Parameter Values can be viewed in \nameref{AppendixA} and \nameref{AppendixE}. 
